...
function AppsPage() {
  ...
  const fetchApps = useCallback(async () => {
    try {
      ...
      setApps(
        appList.map((app: any) => ({ ...app, type: "chatflow" as const })),
      );
    } catch (error) {
  ...
  const fetchWorkflows = useCallback(async () => {
    try {
      ...
      const workflowList = Array.isArray(data)
        ? data
        : data?.workflows || data?.items || data?.data || [];
      setWorkflows(
        workflowList.map((wf: any) => ({ ...wf, type: "workflow" as const })),
      );
    } catch (error) {
  ...
  const fetchKnowledgeBases = useCallback(async () => {
    try {
      ...
      setKnowledgeBases(
        Array.isArray(data) ? data : data?.knowledge_bases || [],
      );
    } catch (error) {
  ...
  const getShareLink = (app: App) => {
    const base = window.location.origin;
    if (app.type === "workflow") {
      return `${base}/share/workflow_${app.id}`;
    }
    return `${base}/share/${app.share_id || app.id}`;
  };

  const handleCopyShareLink = (app: App, e: React.MouseEvent) => {
    e.stopPropagation();
    copyToClipboard(getShareLink(app))
      .then(() => Toast.success("分享链接已复制"))
      .catch(() => Toast.error("复制失败"));
  };

  const getEmbedCode = (app: App) => {
    return `<iframe src="${getShareLink(app)}" width="400" height="600" frameborder="0" style="border:none;border-radius:12px;"></iframe>`;
  };

  const getFullscreenCode = (app: App) =>
    `<iframe\n  src="${getShareLink(app)}"\n  style="width: 100%; height: 100%;"\n  frameborder="0"\n  allow="microphone">\n</iframe>`;

  const getMobileCode = (app: App) =>
    `<iframe\n  src="${getShareLink(app)}?mode=mobile"\n  style="width: 100%; height: 100%;"\n  frameborder="0"\n  allow="microphone">\n</iframe>`;

  const getFloatCode = (app: App) => {
    const origin = window.location.origin;
    return `<script\n  async\n  defer\n  src="${origin}/embed.js?token=${app.share_id || app.id}">\n</script>`;
  };
  ...
  const MobilePreview = () => (
    <div style={{ display: "flex", justifyContent: "center", height: 160 }}>
      <div
        style={{
          width: 90,
          background: "var(--semi-color-fill-0)",
    ...
    <div style={{ padding: 24 }}>
      ...
      >
        ...
        <Space>
          ...
          <Button
            type="primary"
            icon={creatingWorkflow ? <Spin size="small" /> : <IconBranch />}
      ...
      <List
        ...
        emptyContent={
          <Empty
            description={
              searchQuery ? "没有找到匹配的应用" : "还没有创建任何应用"
            }
          />
        }
        renderItem={(app) => (
          ...
          >
            ...
            >
              ...
              {app.icon ? (
                ...
                >
                  {app.type === "workflow" ? (
                    <IconBranch style={{ color: "#fff" }} />
                  ) : (
                    <IconComment style={{ color: "#fff" }} />
                  )}
                </Avatar>
              )}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <Tag
                    size="small"
                    color={app.type === "workflow" ? "green" : "blue"}
                  >
              ...
              >
                {app.type === "chatflow" && (
                  <>
                    ...
                    {app.knowledge_base_ids &&
                      app.knowledge_base_ids.length > 0 && (
                        <Tooltip
                          content={`关联 ${app.knowledge_base_ids.length} 个知识库`}
                        >
                          <Tag
                            size="small"
                            prefixIcon={
                              <IconArchive style={{ fontSize: 10 }} />
                            }
                          >
              ...
              >
                ...
                <Tooltip content="分享">
                  <Button
                    ...
                    size="small"
                    theme="borderless"
                    onClick={(e) => handleCopyShareLink(app, e)}
                  />
                </Tooltip>
      ...
      <Sidebar.Container
        ...
        title={editingApp ? "编辑 Chatflow" : "创建 Chatflow"}
        onCancel={() => {
          setShowCreateModal(false);
          setEditingApp(null);
          resetForm();
      ...
      >
        <div style={{ padding: "16px 0" }}>
          ...
          >
            ...
            <Button
              type="primary"
              onClick={editingApp ? handleUpdateChatflow : handleCreateChatflow}
            >
              {editingApp ? "保存" : "创建"}
            </Button>
          </div>
        ...
        {selectedApp && (
          <div style={{ display: "flex", gap: 16, padding: "8px 0" }}>
            ...
            {renderEmbedCard(
              "全屏模式",
              <FullscreenPreview />,
              getFullscreenCode(selectedApp),
            )}
            {/* 移动端模式 */}
            {renderEmbedCard(
              "移动端模式",
              <MobilePreview />,
              getMobileCode(selectedApp),
            )}
            {/* 浮窗模式 */}
            {renderEmbedCard(
              "浮窗模式",
              <FloatPreview />,
              getFloatCode(selectedApp),
            )}
          </div>
        )}
      </Modal>
